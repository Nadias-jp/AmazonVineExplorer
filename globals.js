...
const SITE_IS_VINE =
  /http[s]{0,1}\:\/\/[w]{0,3}.amazon.[a-z]{1,}.{0,1}[a-z]{0,}\/vine\//.test(
    window.location.href,
  );
const SITE_IS_SHOPPING =
  /http[s]{0,1}\:\/\/[w]{0,3}.amazon.[a-z]{1,}.{0,1}[a-z]{0,}\/(?!vine)(?!gp\/video)(?!music)/.test(
    window.location.href,
  );
...
const INIT_AUTO_SCAN =
  localStorage.getItem("AVE_INIT_AUTO_SCAN") == "true" ? true : false;
const AUTO_SCAN_IS_RUNNING =
  localStorage.getItem("AVE_AUTO_SCAN_IS_RUNNING") == "true" ? true : false;
const AUTO_SCAN_PAGE_CURRENT =
  parseInt(localStorage.getItem("AVE_AUTO_SCAN_PAGE_CURRENT")) || -1;
const AUTO_SCAN_PAGE_MAX =
  parseInt(localStorage.getItem("AVE_AUTO_SCAN_PAGE_MAX")) || -1;
...
function addBranding() {
  ...
  _text.style.backgroundColor = AVE_IS_THIS_SESSION_MASTER
    ? "rgba(218, 247, 166, .75)"
    : "rgba(255, 100, 100, .75)";
  ...
  () => {
    if (!localStorage.getItem("AVE_SESSIONS")) {
      localStorage.setItem(
        "AVE_SESSIONS",
        JSON.stringify([{ id: AVE_SESSION_ID, ts: Date.now() }]),
      );
    } else {
    ...
    setInterval(() => {
      ...
      for (let i = 0; i < _sessions.length; i++) {
        ...
        if (_session.id == AVE_SESSION_ID) {
          ...
          _ownIndex = i;
        } else if (_session.ts + 2500 < Date.now()) {
          // We have found a Invalid Session => Handle this
          if (_session.master && SITE_IS_VINE) {
            // Should we takeover Master ? ONLY IF WE ARE ON VINE SITE
            _noValidMaster = true;
      ...
      if (
        ...
        (_noValidMaster || _sessions.length == 1)
      ) {
...
SETTINGS_USERCONFIG_DEFINES.push({
  type: "title",
  name: "Amazon Vine",
  description: "Tooltip Description of this Setting",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "EnableFullWidth",
  type: "bool",
  name: "Enable Full Width",
  description: "Uses the full width of the display",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DarkMode",
  type: "bool",
  name: "Enable Dark Mode (reload required atm)",
  description:
    "Switches between Amazon Light Theme and AVE Dark Mode (reload required atm)",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DisableAmazonNavbar",
  type: "bool",
  name: "Disable Amazon Navbar",
  description: "Disables the Amazon Navbar",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DisableCategories",
  type: "bool",
  name: "Disable Categories",
  description: "Disables the Categories of the Amazon Vine Page",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DisableFooter",
  type: "bool",
  name: "Disable Footer",
  description: "Disables the Footer of the Amazon Vine Page",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DisableSuggestions",
  type: "bool",
  name: "Disable Suggestions",
  description: "Disables Suggestions on the Amazon Vine Page",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DisableBtnPotLuck",
  type: "bool",
  name: "Disable Button Potluck",
  description: "Disables the Section Button PotLuck(FSE)",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DisableBtnLastChance",
  type: "bool",
  name: "Disable Button Last Chance",
  description: "Disables the Section Button Last Chance(VFA)",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DisableBtnSeller",
  type: "bool",
  name: "Disable Button Seller",
  description: "Disables the Section Button Seller(ZA)",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "EnableTopLogoChange",
  type: "bool",
  name: "Enable Top Logo Change",
  description: "Enables the Change of the top logo to our AVE Logo",
});

SETTINGS_USERCONFIG_DEFINES.push({
  key: "EnableBtnAll",
  type: "bool",
  name: "Enable Button All Products",
  description: 'Enable "All Products" Button',
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "EnableBackgroundScan",
  type: "bool",
  name: "Enable Background Scan",
  description:
    "Enables the Background scan, if disabled you will find a Button for Autoscan on the Vine Website",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "EnableInfiniteScrollLiveQuerry",
  type: "bool",
  name: "Enable Infiniti Scroll Live Querry",
  description:
    "If enabled the Products of the All Products Page will get querryd from Amazon directls otherwise they will get loaded from Database(faster)",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "EnableDesktopNotifikation",
  type: "bool",
  name: "Enable Desktop Notifications",
  description: "Enable Desktop Notifications if new Products are detected",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DesktopNotifikationKeywords",
  type: "keywords",
  name: "Desktop Notification Highlight Keywords",
  inputPlaceholder: "Type in your highlight keyword and press [ENTER]",
  description:
    "Create a List of words u want to Highlight if Product desciption containes one or more of them",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "BackGroundScanDelayPerPage",
  type: "number",
  min: 2000,
  max: 10000,
  name: "Background Scan Per Page Min Delay(Milliseconds)",
  description: "Minimal Delay per Page load of Background Scan",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "BackGroundScannerRandomness",
  type: "number",
  min: 100,
  max: 10000,
  name: "Background Scan Randomness per Page(Milliseconds)",
  description:
    "A Vale that gives the maximal range for the Randomy added delay per page load",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DesktopNotifikationDelay",
  type: "number",
  min: 1,
  max: 900,
  name: "Desktop Notifikation Delay(Seconds)",
  description:
    "Minimal Time between Desktop Notifikations. exept Notifikations with hitted keywords",
});
...
SETTINGS_USERCONFIG_DEFINES.push({
  key: "BtnColorNewProducts",
  type: "color",
  name: "Button Color New Products",
  description: "",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "BtnColorMarkCurrSiteAsSeen",
  type: "color",
  name: "Button Color Mark Current Site As Seen",
  description: "",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "BtnColorMarkAllAsSeen",
  type: "color",
  name: "Button Color Mark All As Seen",
  description: "",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "BtnColorBackToTop",
  type: "color",
  name: "Button Color Back To Top",
  description: "",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "BtnColorUpdateDB",
  type: "color",
  name: "Button Color Update Database",
  description: "",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "BtnColorAllProducts",
  type: "color",
  name: "Button Color All Products",
  description: "",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "BtnColorFavorites",
  type: "color",
  name: "Button Color Favorites",
  description: "",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "FavStarColorDefault",
  type: "color",
  name: "Color Favorite Star unchecked",
  description: "",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "FavStarColorChecked",
  type: "color",
  name: "Color Favorite Star checked",
  description: "",
});
SETTINGS_USERCONFIG_DEFINES.push({
...
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DisableFooterShopping",
  type: "bool",
  name: "Disable Footer",
  description: "Disables the Footer of the Amazon Shopping Page",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DisableSuggestionsShopping",
  type: "bool",
  name: "Disable Suggestions",
  description: "Disables the Suggestions of the Amazon Shopping Page",
});

SETTINGS_USERCONFIG_DEFINES.push({
  type: "title",
  name: "Settings for Developers and Testers",
  description: "",
});
SETTINGS_USERCONFIG_DEFINES.push({
  key: "DebugLevel",
...
SETTINGS_USERCONFIG_DEFINES.push({
  ...
  bgColor: "yellow",
  description:
    "Imports Database from earlyer exported file !! ATTENTION !! At the Moment there is NO VALIDATION CHECK",
  btnClick: () => {
    importDatabase();
  },
});
SETTINGS_USERCONFIG_DEFINES.push({
  ...
  bgColor: "rgb(255,0,0)",
  description:
    "A USER DOES NOT NEED TO DO THIS ! ITS ONLY FOR DEVELOPMENT PURPOSES",
  btnClick: () => {
...
class SETTINGS_DEFAULT {
  ...
  CssProductNewTag =
    "border: 2mm ridge rgba(218, 247, 166, .6); background-color: rgba(218, 247, 166, .2)";
  CssProductSaved =
    "border: 2mm ridge rgba(105, 163, 0, .6); background-color: rgba(105, 163, 0, .2)";
  CssProductFavTag =
    "border: 2mm ridge rgba(255, 255, 102, .6); background-color: rgba(255, 255, 102, .2)";
  CssProductRemovalTag =
    "border: 2mm ridge rgba(255, 87, 51, .6); background-color: rgba(255, 87, 51, .2)";
  CssProductDefault =
    "border: 2mm ridge rgba(173,216,230, .6); background-color: rgba(173,216,230, .2)";
...
function loadSettings() {
  ...
  console.log(
    "Got Settings from GM:(",
    typeof _settingsStore,
    ")",
    _settingsStore,
  );
  if (
    typeof _settingsStore == "object" &&
    _settingsStore != null &&
    _settingsStore != undefined
  ) {
    ...
    for (let i = 0; i < _keysLength; i++) {
      ...
      console.log(
        `Restore Setting: ${_currKey} with Value: ${_settingsStore[_currKey]}`,
      );
...
async function waitForHtmlElmement(selector, cb, altDocument = document) {
  if (typeof selector != "string")
    throw new Error(
      "waitForHtmlElement(): selector is not defined or is not type of string",
    );
  if (typeof cb != "function")
    throw new Error(
      "waitForHtmlElement(): cb is not defined or is not type of string",
    );
...
async function fastStyleChanges() {
  if (SITE_IS_VINE) {
    ...
    if (SETTINGS.DisableAmazonNavbar) {
      ...
      waitForHtmlElmement(
        ".a-container.vvp-body > .a-section:not(#vvp-header)",
        (elem) => {
          elem.style.display = "none";
          // elem.style.visibility = 'hidden';
        },
...
function generateSessionID() {
  return "aaaa-aaaaa-AVE-SESSION-aaaaaaa-aaaaaaaa".replace(/[a]/g, (c) => {
