...
// @version      0.2.5
// @updateURL    https://raw.githubusercontent.com/Nadias-jp/AmazonVineExplorer/main/DataBaseMigrator.user.js
// @downloadURL  https://raw.githubusercontent.com/Nadias-jp/AmazonVineExplorer/main/DataBaseMigrator.user.js
...
try {
  ...
  database.init = (cb) => {
    ...
    database._request.onupgradeneeded = function (event) {
      ...
      if (!_db.objectStoreNames.contains(OBJECT_STORE_NAME)) {
        ...
        const _storeOS = _db.createObjectStore(OBJECT_STORE_NAME, {
          keyPath: "id",
        });
        ...
        switch (
          ...
          default: {
            console.error(
              `There was any Unknown Error while Updating Database from ${event.oldVersion} to ${DATABASE_VERSION}`,
            );
          }
        }
      }
  ...
  database.add = (obj, cb = () => {}) => {
    ...
    const _transaction = database._db.transaction(
      [OBJECT_STORE_NAME],
      "readwrite",
    );
    const _store = _transaction.objectStore(OBJECT_STORE_NAME);
  ...
  vv_database.init = (cb) => {
    vv_database._request = indexedDB.open(
      VV_DATABASE_NAME,
      VV_DATABASE_VERSION,
    );
  ...
  vv_database.getAllKeys = (callback) => {
    ...
    const transaction = vv_database._db.transaction(
      [VV_OBJECT_STORE_NAME],
      "readonly",
    );
...
`;
  ...
  function consolePrintLine(text) {
    consolePrint(`${text || ""}\n`);
  }
    ...
    database.getAll((db) => {
      ...
      element.setAttribute(
        ...
        "data:text/plain;charset=utf-8," +
          encodeURIComponent(JSON.stringify(db, null, 4)),
      );
...
OLD END----------------------------------------------------------*/
  ...
  function finish() {
    ...
    consolePrintLine(
      "export     | To Export and download the converted Database",
    );
    consolePrintLine();
  ...
  function extractDataFromHTML(htmlString) {
    ...
    const dataAsin = tempElement
      .querySelector("input")
      .getAttribute("data-asin");
    const recommendationType = tempElement
      .querySelector("input")
      .getAttribute("data-recommendation-type");
      ...
      vv_database.get(_currKey, (_vvProd) => {
        database.get(_currKey, (_veProd) => {
          ...
          database.update(_veProd, () => {
            _returned++;
            consoleReplace(
              /Updating maching Products.+/,
              `Updating maching Products (${_returned}/${_machingKeys.length})`,
            );
            if (_returned == _machingKeys.length) {
              // All Updates Done ;) Lets start with the "hard" work for conveting VV Products tp VE Products
              consoleReplace(
                /Updating maching Products.+/,
                `Updating maching Products DONE\n`,
              );
              cb();
            }
          });
  ...
  function convertOld(_noMachKeys, cb) {
    ...
    consolePrintLine(
      "Convert Products from Vine Vewer Database to Vine Explorer Database",
    );
    ...
    for (let j = 0; j < _noMachKeys.length; j++) {
      ...
      vv_database.get(_currNmKey, (oldProd) => {
        database.add(convertProduct(oldProd), () => {
          ...
          consoleReplace(
            /Convert Products.+/,
            `Convert Products (${_returned}/${_noMachKeys.length})`,
          );
      ...
      vv_database.init((_ret) => {
        ...
        vv_database.getAllKeys((_vvKeys) => {
          ...
          database.getAllKeys((_veKeys) => {
            ...
            consolePrintLine(
              `found ${_machingKeys.length} Products that are alredy in Vine Explorer Database and ${_noMachKeys.length} who needs to get full converted`,
            );
