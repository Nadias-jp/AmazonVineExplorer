...
// @grant        unsafeWindow
// @require      https://raw.githubusercontent.com/Nadias-jp/AmazonVineExplorer/main/globals.js
// @require      https://raw.githubusercontent.com/Nadias-jp/AmazonVineExplorer/main/class_db_handler.js
// @require      https://raw.githubusercontent.com/Nadias-jp/AmazonVineExplorer/main/class_product.js
...
unsafeWindow.ave = {
  classes: [(DB_HANDLER = DB_HANDLER)],
  config: SETTINGS,
  event: ave_eventhandler,
};

const database = new DB_HANDLER(
  DATABASE_NAME,
      ...
      if (SITE_IS_VINE) {
        ...
        waitForHtmlElmement("body", () => {
          ...
          if (!translation[langCode]) {
            console.error(
              `[AVE] There is no Translation set for the language Code: ${langCode}`,
            );
            langCode = "ja";
          }
        });
        ...
        if (aveData || aveShareData) {
          let _data = aveShareData
            ? JSON.parse(aveShareData)
            : aveData
              ? JSON.parse(aveData)
              : null;
          waitForHtmlElmement("body", () => {
        ...
        waitForHtmlElmement(".vvp-details-btn", () => {
          ...
          const _initialWaitForAllTiles = setInterval(() => {
            const _count =
              document.getElementsByClassName("vvp-details-btn").length; // Buttons take a bit more time as tiles
        ...
        waitForHtmlElmement(".vvp-no-offers-msg", () => {
          // Empty Page ?!?!
          if (_execLock) return;
          _execLock = true;
          if (SETTINGS.DarkMode) {
...
ave_eventhandler.on("ave-database-changed", () => {
  console.warn(
    "EVENT - Database has new Data for us! we should look what has changed",
  );
...
function handleInfiniteScroll() {
  ...
  if (!inifiniteScrollBlockAppend) {
    ...
    appendInfiniteScrollTiles(() => {
      inifiniteScrollBlockAppend = false;
    });
  }
  ...
  if (SETTINGS.EnableInfiniteScrollLiveQuerry) {
    ...
    const _maxScrollHeight = Math.max(
      document.body.scrollHeight - window.innerHeight,
      document.documentElement.scrollHeight - window.innerHeight,
    );

    console.log(
      `handleInfiniteScroll(): _maxScrollHeight: ${_maxScrollHeight} window.scrollY+inner: ${window.scrollY + window.innerHeight}`,
    );
    ...
    if (_maxScrollHeight > window.scrollY + window.innerHeight * 2) {
      ...
      infiniteScrollTilesBufferArray.length < 1000 &&
      infiniteScrollLastPreloadedPage < infiniteScrollMaxPreloadPage
    ) {
      const _baseUrl =
        /(http[s]{0,1}\:\/\/[w]{0,3}.amazon.[a-z]{1,}.{0,1}[a-z]{0,}\/vine\/vine-items)/.exec(
          window.location.href,
        )[1];
      infiniteScrollLastPreloadedPage++;
      getTilesFromURL(
        `${_baseUrl}?queue=encore&pn=&cn=&page=${infiniteScrollLastPreloadedPage}`,
        (tiles) => {
          infiniteScrollTilesBufferArray =
            infiniteScrollTilesBufferArray.concat(tiles);

          blockHandleInfiniteScroll = false;
          if (infiniteScrollTilesBufferArray.length < 500)
...
function detectCurrentPageType() {
  if (
    /http[s]{0,1}\:\/\/[w]{0,3}.amazon.[a-z]{1,}.{0,1}[a-z]{0,}\/vine\/vine-items$/.test(
      window.location.href,
    )
  ) {
...
async function parseTileData(tile) {
  return new Promise((resolve, reject) => {
    if (SETTINGS.DebugLevel > 5)
      console.log(`Called parseTileData(`, tile, ")");
    ...
    database.get(_id).then((_ret) => {
      if (_ret) {
        ...
        if (SETTINGS.DebugLevel > 14)
          console.log(`parseTileData(): got DB Entry`);
        ...
        waitForHtmlElmement(
          ".vvp-item-tile-content",
          async () => {
            const _div_vpp_item_tile_content = tile.getElementsByClassName(
              "vvp-item-tile-content",
            )[0];
            if (SETTINGS.DebugLevel > 14)
              console.log(`parseTileData(): wait 1`);
            waitForHtmlElmement(
              "img",
              async () => {
                const _div_vpp_item_tile_content_img =
                  _div_vpp_item_tile_content.getElementsByTagName("img")[0];
                if (SETTINGS.DebugLevel > 14)
                  console.log(`parseTileData(): wait 2`);
                waitForHtmlElmement(
                  ".vvp-item-product-title-container",
                  async () => {
                    const _div_vvp_item_product_title_container =
                      _div_vpp_item_tile_content.getElementsByClassName(
                        "vvp-item-product-title-container",
                      )[0];
                    if (SETTINGS.DebugLevel > 14)
                      console.log(`parseTileData(): wait 3`);
                    waitForHtmlElmement(
                      "a",
                      async () => {
                        const _div_vvp_item_product_title_container_a =
                          _div_vvp_item_product_title_container.getElementsByTagName(
                            "a",
                          )[0];
                        if (SETTINGS.DebugLevel > 14)
                          console.log(`parseTileData(): wait 4`);
                        waitForHtmlElmement(
                          ".a-button-inner",
                          async () => {
                            const _div_vpp_item_tile_content_button_inner =
                              _div_vpp_item_tile_content.getElementsByClassName(
                                "a-button-inner",
                              )[0];
                            if (SETTINGS.DebugLevel > 14)
                              console.log(`parseTileData(): wait 5`);
                            waitForHtmlElmement(
                              "input",
                              async () => {
                                const _div_vpp_item_tile_content_button_inner_input =
                                  _div_vpp_item_tile_content_button_inner.getElementsByTagName(
                                    "input",
                                  )[0];
                                if (SETTINGS.DebugLevel > 14)
                                  console.log(`parseTileData(): wait 6`);
                                const _newProduct = new Product(_id);
                                _newProduct.data_recommendation_id = _id;
                                _newProduct.data_img_url =
                                  tile.getAttribute("data-img-url");
                                _newProduct.data_img_alt =
                                  _div_vpp_item_tile_content_img.getAttribute(
                                    "alt",
                                  ) || "";
                                _newProduct.link =
                                  _div_vvp_item_product_title_container_a.getAttribute(
                                    "href",
                                  );
                                _newProduct.description_full =
                                  _div_vvp_item_product_title_container_a.getElementsByClassName(
                                    "a-truncate-full",
                                  )[0].textContent;

                                _newProduct.data_asin =
                                  _div_vpp_item_tile_content_button_inner_input.getAttribute(
                                    "data-asin",
                                  );
                                _newProduct.data_recommendation_type =
                                  _div_vpp_item_tile_content_button_inner_input.getAttribute(
                                    "data-recommendation-type",
                                  );
                                _newProduct.data_asin_is_parent =
                                  _div_vpp_item_tile_content_button_inner_input.getAttribute(
                                    "data-is-parent-asin",
                                  ) == "true";

                                _newProduct.description_short =
                                  _div_vvp_item_product_title_container_a.getElementsByClassName(
                                    "a-truncate-cut",
                                  )[0].textContent;
                                ...
                                if (_newProduct.description_short == "") {
                                  if (SETTINGS.DebugLevel > 14)
                                    console.log(
                                      `parseTileData(): we don´t have a shot description`,
                                    );
                                  let _timeLoopCounter = 0;
                                  const _maxLoops = Math.round(
                                    SETTINGS.FetchRetryMaxTime /
                                      SETTINGS.FetchRetryTime,
                                  );
                                  const _halfdelay =
                                    SETTINGS.FetchRetryTime / 2;
                                  function timeLoop() {
...
function addLeftSideButtons(forceClean) {
  ...
  const _setAllSeenBtn = createButton(
    translation[langCode]["ave-btn-allseen"],
    "ave-btn-allseen",
  ...
  const _setAllSeenDBBtn = createButton(
    translation[langCode]["ave-btn-db-allseen"],
    "ave-btn-db-allseen",
  ...
  const _backToTopBtn = createButton(
    translation[langCode]["ave-btn-backtotop"],
    "ave-btn-backtotop",
...
function markAllCurrentDatabaseProductsAsSeen(cb = () => {}) {
  if (SETTINGS.DebugLevel > 10)
    console.log("Called markAllCurrentDatabaseProductsAsSeen()");
  database.getNewEntries().then((prods) => {
    const _prodsLength = prods.length;
    let _returned = 0;
    if (SETTINGS.DebugLevel > 10)
      console.log(
        `markAllCurrentDatabaseProductsAsSeen() - Got ${_prodsLength} Products with Tag isNew`,
      );
    if (_prodsLength == 0) {
      cb(true);
      return;
    ...
    for (let i = 0; i < _prodsLength; i++) {
      ...
      database.update(_currProd, () => {
        if (SETTINGS.DebugLevel > 10)
          console.log(
            `markAllCurrentDatabaseProductsAsSeen() - Updated ${_currProd.id}`,
          );
        _returned++;
...
function createButton(text, id, style, clickHandler) {
  ...
  _btnSpan.innerHTML = `
        <span class="a-button-inner" style="${style || ""}">
            <span class=" data-ave-key="${id}">${text}</span>
        </span>
    `;
...
async function createTileFromProduct(product, btnID, cb) {
  if (!product && SETTINGS.DebugLevel > 10)
    console.error(`createTileFromProduct got no valid product element`);
  return new Promise((resolve, reject) => {
    ...
    _tile.setAttribute(
      ...
      product.data_recommendation_id,
    );
    _tile.setAttribute("data-img-url", product.data_img_url);
    _tile.setAttribute(
      "style",
      product.notSeenCounter > 0
        ? SETTINGS.CssProductRemovalTag
        : product.isFav
          ? SETTINGS.CssProductNewTag
          : product.isNew
            ? SETTINGS.CssProductNewTag
            : SETTINGS.CssProductDefault,
    );
    _tile.innerHTML = `
            <div class="vvp-item-tile-content">
                ...
                <span class="a-button a-button-primary vvp-details-btn" id="a-autoid-${_btnAutoID}">
                    <span class="a-button-inner">
                        ...
                        <span class="a-button-text" aria-hidden="true" id="a-autoid-${_btnAutoID}-announce">${translation[langCode]["ave-more-details"]}</span>
                    </span>
                </span>
            </div>
        `;
...
function createFavStarElement(prod, index = Math.round(Math.random() * 10000)) {
  ...
  _favElement.setAttribute(
    "id",
    `p-fav-${index || Math.round(Math.random() * 5000)}`,
  );
...
function createShareElement(prod, index = Math.round(Math.random() * 10000)) {
  ...
  _shareElement.setAttribute(
    "id",
    `ave-p-share-${index || Math.round(Math.random() * 5000)}`,
  );
...
function shareEventHandlerClick(event, _data) {
  if (_data.recommendation_id) {
    ...
    const newUrl = `${window.location.origin}/dp/${_data.asin}?vine-data=${encodeURIComponent(
      JSON.stringify({
        asin: _data.asin,
...
function createTaxInfoElement(prod, index = Math.round(Math.random() * 10000)) {
  ...
  let _currencySymbol = "";
  if (prod.data_tax_currency && prod.data_tax_currency == "EUR")
    _currencySymbol = "€";

  const _taxElement = document.createElement("span");
  _taxElement.setAttribute("id", `ave-taxinfo-${index}`);
  _taxElement.style.cssText =
    "position: relative; transform: translate(0px, -30px); width: fit-content; right: 0px;";
  ...
  _taxElement_span.innerText = `${translation[langCode]["ave-tax-prize"]} ${typeof _prize == "number" ? _prize : "--.--"} ${_currencySymbol}`;
...
async function createProductSite(siteType, productArray, cb) {
  ...
  if (SETTINGS.DebugLevel > 10)
    console.log(`Create Overview for ${_productArrayLength} Products`);

  // Remove Pagination
  const _pagination = document.querySelector(".a-pagination");
  if (_pagination) _pagination.remove();

  const _contentContainer = document.querySelector(
  ...
  if (_tilesContainer) {
    ...
    _topLine.innerHTML = `<p>${translation[langCode]["ave-show-results"][1]} <strong>${_fastCount}</strong> von <strong>${_productArrayLength}</strong> Ergebnissen</p>`;
  }
  ...
  for (; _index < _fastCount; _index++) {
    createTileFromProduct(productArray[_index], _index, (tile) => {
      ...
      _returned++;
      if (SETTINGS.DebugLevel > 10)
        console.log(`Created Tile (${_returned}/${_fastCount})`);
...
async function createInfiniteScrollSite(siteType, cb) {
  if (SETTINGS.DebugLevel > 10)
    console.log(`Called createInfiniteScrollSite()`);
  ...
  while (infiniteScrollTilesBufferArray.length > 0 && !_stopCreation) {
    ...
    if (SETTINGS.EnableInfiniteScrollLiveQuerry) {
      ...
      parseTileData(_tile).then((_product) => {
        if (SETTINGS.DebugLevel > 14)
          console.log(
            "Come Back from parseTileData <<<<<<<<<< INFINITYSCROLL <<<<<<<<<<<<<<<<<<<<<<<",
            _tile,
            _product,
          );
    ...
    console.log(
      `appendInfiniteScrollTiles(): Inside WHILE: _maxScrollHeigt: ${_maxScrollHeight} currPosition ${window.scrollY}`,
    );
  }

  console.log(
    `appendInfiniteScrollTiles(): After WHILE: left tile to create: ${infiniteScrollTilesBufferArray.length}`,
  );
  cb(true);
      ...
      createInfiniteScrollSite(currentMainPage, (tilesContainer) => {
        ...
        if (SETTINGS.EnableInfiniteScrollLiveQuerry) {
          getTilesFromURL(`${_baseUrl}?queue=${_preloadPages[0]}`, (tiles1) => {
            infiniteScrollTilesBufferArray =
              infiniteScrollTilesBufferArray.concat(tiles1);
            appendInfiniteScrollTiles();
            getTilesFromURL(
              `${_baseUrl}?queue=${_preloadPages[1]}`,
              (tiles2) => {
                infiniteScrollTilesBufferArray =
                  infiniteScrollTilesBufferArray.concat(tiles2);
                appendInfiniteScrollTiles();
                getTilesFromURL(
                  `${_baseUrl}?queue=${_preloadPages[2]}`,
                  (tiles3) => {
                    infiniteScrollTilesBufferArray =
                      infiniteScrollTilesBufferArray.concat(tiles3);
                    appendInfiniteScrollTiles();
                    setTimeout(() => {
...
function getTilesFromURL(url, cb = (tilesArray) => {}) {
  if (lastGetTilesFromURLQuerry + SETTINGS.PageLoadMinDelay > Date.now()) {
    const _delay = Math.max(
      1,
      lastGetTilesFromURLQuerry + SETTINGS.PageLoadMinDelay - Date.now(),
    );
    console.warn(`getTilesFromURL() DELAYED for ${_delay}ms`);
    setTimeout(() => {
      getTilesFromURL(url, cb);
    }, _delay);
    return;
  }
  GM.xmlHttpRequest({
    ...
    onload: function (response) {
      ...
      waitForHtmlElmement(
        ...
        (itemsContainer) => {
          ...
          if (_paginationData)
            infiniteScrollMaxPreloadPage = _pageinationData.maxPage;
        },
        _doc,
      );
    },
...
function btnEventhandlerClick(event, data) {
  ...
  if (SETTINGS.DebugLevel > 10)
    console.log(
      `called btnEventhandlerClick(${JSON.stringify(event)}, ${JSON.stringify(data)})`,
    );
  if (data.recommendation_id) {
    database.get(data.recommendation_id).then(async (prod) => {
      if (SETTINGS.DebugLevel > 10)
        console.log(`btnEventhandlerClick() got respose from DB:`, prod);
      if (prod) {
        prod.isNew = false;
...
function favStarEventhandlerClick(event, data) {
  if (SETTINGS.DebugLevel > 10)
    console.log(
      `called favStarEventhandlerClick(${JSON.stringify(event)}, ${JSON.stringify(data)})`,
    );
  if (data.recommendation_id) {
    database.get(data.recommendation_id).then((prod) => {
      if (SETTINGS.DebugLevel > 10)
        console.log(`favStarEventhandlerClick() got respose from DB:`, prod);
      if (prod) {
...
function updateTileStyle(prod) {
  if (SETTINGS.DebugLevel > 10)
    console.log(`Called updateTileStyle(${JSON.stringify(prod, null, 4)})`);
  const _tiles = document.getElementsByClassName("vvp-item-tile");
  const _tilesLength = _tiles.length;

  if (SETTINGS.DebugLevel > 10)
    console.log(`Searching for tile with id ${prod.id}`);
  for (let i = 0; i < _tilesLength; i++) {
    const _tile = _tiles[i];
    const _id = _tile.getAttribute("data-recommendation-id");
    ...
    if (_id == prod.data_recommendation_id) {
      if (SETTINGS.DebugLevel > 10)
        console.log(`Found Tile with id: ${prod.id}`);
      _tile.setAttribute(
        "style",
        prod.isFav
          ? SETTINGS.CssProductFavTag
          : prod.isNew
            ? SETTINGS.CssProductNewTag
            : SETTINGS.CssProductDefault,
      );
      const _favStar = _tile.querySelector(".ave-favorite-star");
      _favStar.style.color = prod.isFav
        ? SETTINGS.FavStarColorChecked
        : "white"; // SETTINGS.FavStarColorChecked = Gelb;
...
function initTileEventHandlers() {
  if (SETTINGS.DebugLevel > 10)
    console.log(
      "Called inttTileEventHandlers() >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>",
    );
  const _tiles = document.getElementsByClassName("vvp-item-tile");
  const _tileLength = _tiles.length;
  for (let i = 0; i < _tileLength; i++) {
    if (SETTINGS.DebugLevel > 10)
      console.log(`Adding Eventhandler to Tile ${i}`);
...
function addTileEventhandlers(_currTile) {
  ...
  waitForHtmlElmement('[id^="ave-taxinfo-"]', (elem) => {
    _data.tax = _currTile.querySelector(
      '[id^="ave-taxinfo-"] > span',
    ).textContent;
  });
  ...
  _btn.addEventListener("click", (event) => {
    btnEventhandlerClick(event, _data);
  });

  for (let j = 0; j < _childs.length; j++) {
    if (SETTINGS.DebugLevel > 10)
      console.log(`Adding Eventhandler to Children ${j} of Tile ${i}`);
    _childs[j].addEventListener("click", (event) => {
      btnEventhandlerClick(event, _data);
    });
  }
  ...
  waitForHtmlElmement(
    ...
    (elem) => {
      elem.addEventListener("click", (event) => {
        favStarEventhandlerClick(event, _data);
      });
    },
    _currTile,
  );
  ...
  waitForHtmlElmement(
    ...
    (elem) => {
      elem.addEventListener("click", (event) => {
        shareEventHandlerClick(event, _data);
      });
    },
    _currTile,
  );
...
function addAveSettingsTab() {
  waitForHtmlElmement(
    ".vvp-tab-set-container > ul",
    (_upperButtonsContainer) => {
      ...
      _upperSettingsButton.innerHTML += `<a role="tab" aria-selected="false" tabindex="-1">AVE ${translation[langCode]["ave-settings"]}</a>`;

      _upperSettingsButton.addEventListener("click", function () {
        const _upperButtons = document.body.querySelectorAll(
          ".a-tab-container.vvp-tab-set-container > ul > li",
        );
        _upperButtons.forEach((element) =>
          element.classList.remove("a-active"),
        );

        const _contentContainer = document.body.querySelectorAll(
          ".a-tab-container.vvp-tab-set-container > div",
        );
        _contentContainer.forEach((element) =>
          element.classList.add("a-hidden"),
        );
        console.log(_contentContainer);

        _upperSettingsButton.classList.add("a-active");
        const _settingsContent = document.body.querySelector(
          '[data-a-name="ave-settings"]',
        );
        _settingsContent.classList.remove("a-hidden");

        const _settingsContainer = _settingsContent.querySelector(
...
function addAVESettingsMenu() {
  waitForHtmlElmement(
    ".a-tab-container.vvp-tab-set-container",
    (_tabContainer) => {
...
::-moz-focus-inner{
  padding: 0;
    ...
    <div id="ave-settings-header" style="margin-bottom: 10px"><h3>${translation[langCode]["ave-settings"]} ${AVE_TITLE} - Version ${AVE_VERSION}</h3></div>
...
function createSettingsMenuElement(dat) {
  ...
  if (dat.type == "bool") {
    ...
    _elem_item_left_label_input.addEventListener("click", (event) => {
      console.log("This is a Boolean Value Input", event);
      SETTINGS[dat.key] = event.target.checked;
      SETTINGS.save();
    });

    const _elem_item_left_label_span = document.createElement("span");
    _elem_item_left_label_span.classList.add(
      "ave-settings-switch-toggle-slider",
    );
    ...
    _elem_item_left_input_label.setAttribute(
      "data-ave-tooltip",
      dat.description && dat.description != "" ? dat.description : dat.name,
    );
    _elem_item_left_input_label.setAttribute("class", "a-button");
    _elem_item_left_input_label.style.width = "250px";
    if (dat.bgColor)
      _elem_item_left_input_label.style.backgroundColor = dat.bgColor;
    ...
    _elem_item_left_input.addEventListener("click", (event) => {
      console.log("This is a button Input", event);
      if (dat.btnClick) dat.btnClick();
    });
    ...
    _elem_item_left_input.addEventListener("change", (event) => {
      console.log("This is a Color Value Input", event);
    ...
    _elem_keyword_input_input.addEventListener("change", (elm, ev) => {
      ...
      if (_value && _value.length > 0 && !SETTINGS[dat.key].includes(_value))
        SETTINGS[dat.key].push(_value);
      SETTINGS.save();
      elm.target.value = "";
      ...
      for (let i = 0; i < SETTINGS[dat.key].length; i++) {
        _table.appendChild(
          createSettingsKeywordsTableElement(dat, i, SETTINGS[dat.key][i]),
        );
      }
    });
    ...
    for (let i = 0; i < SETTINGS[dat.key].length; i++) {
      _elem_keyword_list_table_tbody.appendChild(
        createSettingsKeywordsTableElement(dat, i, SETTINGS[dat.key][i]),
      );
    }
    ...
    <div style="background-color: white;border-radius: 8px;width: 50%;min-width: 250px;height: 75%;overflow: hidden;">
      <div id="settingsInner"width: 100%; height: 100%;"> <!--- Inner Start -->
        <div id="settingsNav" style="background-color: #F0F2F2;border-bottom: 1px solid #D5D9D9;display: flex;height: 50px;align-items: center;padding: 0 24px;"> <!--- Nav Start -->
         <div style="color: #444;font-size: 16px;font-weight: 700;">Amazon Vine Explorer ${translation[langCode]["ave-settings"]}</div>
...
function getPageinationData(localDocument = document) {
  ...
  while ((!_ret.href || !_ret.maxPage) && _currChild) {
    ...
    if (_curr.hasAttribute("href"))
      _ret.href = _curr.getAttribute("href").replace(/=[0-9]+/, "=");
...
async function cleanUpDatabase(cb = () => {}) {
  ...
  database.getAll().then((prodArr) => {
    ...
    const _workersProms = [];
    if (SETTINGS.DebugLevel > 10)
      console.log(`cleanUpDatabase() - Checking ${_prodArrLength} Entrys`);
    ...
    for (const _currEntry of prodArr) {
      _workersProms.push(
        new Promise((resolve, reject) => {
          ...
          if (SETTINGS.DebugLevel > 10)
            console.log(`cleanUpDatabase() - Checking Entry ${_currEntry.id} `);

          // Checking Product Vars
          if (!_currEntry.ts_firstSeen) {
            _currEntry.ts_firstSeen =
              unixTimeStamp() -
              Math.round(Math.random() * (SECONDS_PER_WEEK / 2));
            _needUpdate = true;
            if (SETTINGS.DebugLevel > 14)
              console.log(
                `cleanUpDatabase() - Entry ${_currEntry.id} had no valid firstseen timestamp. fixed`,
              );
          }
          ...
          if (!_currEntry.ts_lastSeen) {
            ...
            if (SETTINGS.DebugLevel > 14)
              console.log(
                `cleanUpDatabase() - Entry ${_currEntry.id} had no valid lastseen timestamp. fixed`,
              );
          }

          let _notSeenCounter = _currEntry.notSeenCounter;
          if (
            _currEntry.data_recommendation_type == "VENDOR_TARGETED" &&
            _currEntry.ts_lastSeen < unixTimeStamp() - SECONDS_PER_DAY
          ) {
            // If PotLuck start revoving after 1 day
            _notSeenCounter++;
            if (SETTINGS.DebugLevel > 14)
              console.log(
                `cleanUpDatabase() - Entry ${_currEntry.id} increased notSeenCounter to ${_notSeenCounter}`,
              );
          } else if (
            _currEntry.ts_lastSeen <
            unixTimeStamp() - SECONDS_PER_WEEK
          ) {
            // Normal Product Start Removing after 1 week
            _notSeenCounter++;
            if (SETTINGS.DebugLevel > 14)
              console.log(
                `cleanUpDatabase() - Entry ${_currEntry.id} increased notSeenCounter to ${_notSeenCounter}`,
          ...
          if (_currEntry.notSeenCounter != _notSeenCounter) {
            if (SETTINGS.DebugLevel > 14)
              console.log(
                `cleanUpDatabase() - Entry ${_currEntry.id} update notSeenCounter from ${_currEntry.notSeenCounter} to ${_notSeenCounter}`,
              );
          ...
          if (
            (_currEntry.notSeenCounter > SETTINGS.NotSeenMaxCount ||
              _currEntry.forceRemove) &&
            !_currEntry.isFav
          ) {
            if (SETTINGS.DebugLevel > 10)
              console.log(
                `cleanUpDatabase() - Removing Entry ${_currEntry.id}`,
              );
            ...
            database.update(_currEntry).then((ret) => {
              _updated++;
              resolve();
            });
          }
    ...
    Promise.allSettled(_workersProms).then(() => {
      if (SETTINGS.DebugLevel > 0)
        console.log(
          `Databasecleanup Finished: Entrys:${_prodArrLength} Updated:${_updated} Deleted:${_deleted}`,
        );
      _dbCleanIcon.remove();
      cb(true);
    });
...
function exportDatabase() {
  ...
  database.getAll().then((db) => {
    try {
      ...
      const dbBlob = new Blob([JSON.stringify(db, null, 4)], {
        type: "application/json;charset=utf-8",
      });

      console.log(
        "Emulating download file using saveAs script to export JSON file (uncompressed)",
      );
        ...
        try {
          ...
          for (const data of jsonData) {
            ...
            if (!existingRecord) {
              ...
              console.warn(
                `Record with ID ${data.id} already exists. Skipping.`,
              );
            }
          }
...
function initBackgroundScan() {
  ...
  if (BackGroundScanIsRunning) {
    console.warn(
      "initBackgroundScan(): Backgroundscan is already running => Exit",
    );
    return;
  }
  if (!SETTINGS.EnableBackgroundScan) {
    console.warn("initBackgroundScan(): Backgroundscan is disabled => Exit");
    return;
  }
  if (!AVE_IS_THIS_SESSION_MASTER) {
    console.warn(
      "initBackgroundScan(): This Instance is not the Master Session! => don´t start BackgroundScan",
    );
    return;
  }
  ...
  if (!document.querySelector("#ave-iframe-backgroundloader")) {
    if (SETTINGS.DebugLevel > 10)
      console.log("initBackgroundScan(): create iFrame");
  ...
  const _paginatinWaitLoop = setInterval(() => {
    const _pageinationData = getPageinationData(
      document.querySelector("#ave-iframe-backgroundloader").contentWindow
        .document,
    );
    if (_pageinationData) {
      clearInterval(_paginatinWaitLoop);
      if (SETTINGS.DebugLevel > 10)
        console.log("initBackgroundScan(): pagination WaitLoop");

      if (!(localStorage.getItem("AVE_BACKGROUND_SCAN_IS_RUNNING") == "true")) {
        if (SETTINGS.DebugLevel > 10)
          console.log("initBackgroundScan(): init localStorage Variables");
        localStorage.setItem(
          "AVE_BACKGROUND_SCAN_PAGE_MAX",
          _pageinationData.maxPage,
        );
      ...
      backGroundScanTimeout = setTimeout(
        initBackgroundScanSubFunctionScannerLoop,
      ...
      function initBackgroundScanSubFunctionScannerLoop() {
        ...
        let _backGroundScanStage =
          parseInt(localStorage.getItem("AVE_BACKGROUND_SCAN_STAGE")) || 0;
        if (SETTINGS.DebugLevel > 10)
          console.log(
            "initBackgroundScan(): loop with _backgroundScanStage ",
        ...
        switch (_backGroundScanStage) {
          case 0: {
            ...
            if (SETTINGS.DebugLevel > 10)
              console.log(
                "initBackgroundScan().loop.case.0 with _subStage: ",
                _subStage,
              );
            if (_stageZeroSites[_subStage]) {
              ...
              backGroundTileScanner(
                `${_baseUrl}?${_stageZeroSites[_subStage]}`,
                (elm) => {
                  _scanFinished();
                },
              );
          ...
          case 1: {
            // queue=encore | queue=encore&pn=&cn=&page=2...x
            _subStage = parseInt(
              localStorage.getItem("AVE_BACKGROUND_SCAN_PAGE_CURRENT"),
            );
            if (SETTINGS.DebugLevel > 10)
              console.log(
                "initBackgroundScan().loop.case.1 with _subStage: ",
                _subStage,
              );
            if (
              _subStage <
              (parseInt(localStorage.getItem("AVE_BACKGROUND_SCAN_PAGE_MAX")) ||
                0)
            ) {
              backGroundTileScanner(
                `${_baseUrl}?queue=encore&pn=&cn=&page=${_subStage + 1}`,
                () => {
                  _scanFinished();
                },
              );
              _subStage++;
              localStorage.setItem(
                "AVE_BACKGROUND_SCAN_PAGE_CURRENT",
                _subStage,
              );
            } else {
              _subStage = 0;
          ...
          case 2: {
            // qerry about other values (tax, real prize, ....) ~ 20 - 30 Products then loopover to stage 1
            ...
            if (SETTINGS.DebugLevel > 10)
              console.log(
                "initBackgroundScan().loop.case.2 with _subStage: ",
                _subStage,
              );
            database.getAll().then((products) => {
              ...
              for (const _prod of products) {
                if (_needUpdate.length < _randCount) {
                  if (typeof _prod.data_estimated_tax_prize != "number")
                    _needUpdate.push(_prod);
                } else {
                  break;
                }
              }
              ...
              Promise.all(_promises)
                ...
                .catch(() => {
                  console.error(
                    "There was an error while updating an product in database",
                  );
                  _scanFinished();
                  _subStage++;
                });
        ...
        function _scanFinished() {
          if (SETTINGS.DebugLevel > 10)
            console.log(`initBackgroundScan()._scanFinished()`);
          localStorage.setItem(
            "AVE_BACKGROUND_SCAN_STAGE",
            _backGroundScanStage,
          );
          ...
          backGroundScanTimeout = setTimeout(
            initBackgroundScanSubFunctionScannerLoop,
            SETTINGS.BackGroundScanDelayPerPage +
              Math.round(Math.random() * SETTINGS.BackGroundScannerRandomness),
          );
        }
      }
    }
...
function backGroundTileScanner(url, cb) {
  if (SETTINGS.DebugLevel > 10)
    console.log(`Called backgroundTileScanner(${url})`);
  const _iconLoading = addLoadingSymbol();
  const _iframeDoc = document.querySelector("#ave-iframe-backgroundloader")
    .contentWindow.document;
  ave.backGroundIFrame = _iframeDoc;
  _iframeDoc.location.href = url;
  const _loopDelay = setInterval(() => {
    if (SETTINGS.DebugLevel > 10)
      console.log(`backgroundTileScanner(): check if we have tiles to read...`);
    const _tiles = _iframeDoc.querySelectorAll(".vvp-item-tile");
    if (_tiles) {
      if (SETTINGS.DebugLevel > 10)
        console.log(`backgroundTileScanner(): Found first Tile`);
      const _tilesLength = _tiles.length;
      if (SETTINGS.DebugLevel > 10)
        console.log(
          `BackgroundsScan Querryd: ${url} and got ${_tilesLength} Tiles`,
        );
      ...
      if (_tilesLength > 0) {
        ...
        for (let i = 0; i < _tilesLength; i++) {
          _tilesProm.push(
            parseTileData(_tiles[i]).then((prod) => {
              _returned++;
              if (SETTINGS.DebugLevel > 14)
                console.log(
                  `BACKGROUNDSCAN => Got TileData Back: Tile ${_returned}/${_tilesLength} =>`,
                  prod,
                );
        ...
        if (SETTINGS.DebugLevel > 10)
          console.log(
            `BACKGROUNDSCAN => We dont have anything to do here anything => resume autoscan`,
          );
...
function startAutoScan() {
  ...
  markAllCurrentDatabaseProductsAsSeen(() => {
    if (SETTINGS.DebugLevel > 10)
      console.log(
        "startAutoScan() - Got Callback from markAllCurrentDatabaseProductsAsSeen()",
      );
...
function handleAutoScan() {
  ...
  const _delay =
    Math.max(
      SETTINGS.PageLoadMinDelay - (Date.now() - PAGE_LOAD_TIMESTAMP),
      0,
    ) + 500;
  if (SETTINGS.DebugLevel > 10)
    console.log(`handleAutoScan() - _delay: ${_delay}`);
...
function stickElementToTopScrollEVhandler(elemID, dist) {
  ...
  if (_elem) {
    ...
    requestAnimationFrame(() => {
      ...
      const _elemInitialTop = parseInt(
        _elem.getAttribute("ave-data-default-top"),
      );
      if (!_elemInitialTop) {
        _elem.setAttribute(
          "ave-data-default-top",
          window.scrollY + _elemRect.top,
        );
        return;
      }
      ...
      if (SETTINGS.DebugLevel > 10)
        console.log(
          `### scrollY:${window.scrollY} maxScrollHeigt ${maxScrollHeight} initialTop: ${_elemInitialTop}`,
        );
...
function updateNewProductsBtn() {
  ...
  database.getNewEntries().then((prodArr) => {
    ...
    if (SETTINGS.DebugLevel > 1)
      console.log(
        `updateNewProductsBtn(): Got Database Response: ${_prodArrLength} New Items`,
      );
    ...
    let _notifyed = false;
    if (
      SETTINGS.EnableDesktopNotifikation &&
      SETTINGS.DesktopNotifikationKeywords?.length > 0
    ) {
      if (SETTINGS.DebugLevel > 1)
        console.log(`updateNewProductsBtn(): Insige IF`);
      ...
      for (let i = 0; i < _prodArrLength; i++) {
        ...
        if (SETTINGS.DebugLevel > 1)
          console.log(
            `updateNewProductsBtn(): Search Product Deescription: ${_descFull} for keys: `,
            _configKeyWords,
          );
        ...
        for (let j = 0; j < _configkeyWordsLength; j++) {
          ...
          if (SETTINGS.DebugLevel > 1)
            console.log(
              `updateNewProductsBtn(): Search Product Deescription for Keyword: ${_currKey}`,
            );
          if (_descFull.includes(_currKey)) {
            desktopNotifikation(
              `Amazon Vine Explorer - ${AVE_VERSION}`,
    ...
    if (
      SETTINGS.EnableDesktopNotifikation &&
      !_notifyed &&
      _prodArrLength > oldCountOfNewItems
    ) {
      if (
        unixTimeStamp() - lastDesktopNotifikationTimestamp >=
        SETTINGS.DesktopNotifikationDelay
      ) {
        ...
        desktopNotifikation(
          `Amazon Vine Explorer - ${AVE_VERSION}`,
          `Es wurden ${_prodArrLength} neue Vine Produkte gefunden`,
        );
      }
    }
  });
  ...
  if (Notification.permission === "granted") {
    const _notification = new Notification(title, {
      ...
      tag: requireInteraction
        ? `ave-notify-${Math.round(Math.random() * 10000000)}`
        : "ave-notify",
...
function addStyleToTile(_currTile, _product) {
  if (!_product.gotFromDB) {
    // We have a new one ==> Save it to our Database ;)
...
async function requestProductDetails(prod) {
  return new Promise(async (resolve, reject) => {
    if (prod.data_asin_is_parent) {
      // Lets get the Childs first
      fetch(
        `${window.location.origin}/vine/api/recommendations/${prod.id}`.replace(
          /#/g,
          "%23",
        ),
      )
        ...
        .then(async (res) => {
          ...
          for (_child of prod.data_childs) {
            _promArray.push(
              fetch(
                `${window.location.origin}/vine/api/recommendations/${prod.id.replace(/#/g, "%23")}/item/${_child.asin}`.replace(
                  /#/g,
                  "%23",
                ),
              )
      ...
      fetch(
        `${window.location.origin}/vine/api/recommendations/${prod.id}/item/${prod.data_asin}`.replace(
          /#/g,
          "%23",
        ),
      )
...
function init(hasTiles) {
  ...
  if (AUTO_SCAN_IS_RUNNING)
    showAutoScanScreen(
      `Autoscan is running...Page (${AUTO_SCAN_PAGE_CURRENT}/${AUTO_SCAN_PAGE_MAX})`,
    );

  const _aveSubpageRequest = getUrlParameter("ave-subpage");
  if (SETTINGS.DebugLevel > 10)
    console.log(`Got Subpage Parameter`, _aveSubpageRequest);
  ...
  if (hasTiles) {
    ...
    for (let i = 0; i < _tilesLength; i++) {
      ...
      _tilePorms.push(
        parseTileData(_currTile).then((_product) => {
          if (SETTINGS.DebugLevel > 14)
            console.log(
              "Come Back from parseTileData <<<<<<<<<< INIT <<<<<<<<<<<<<<<<<<<<<<<",
              _currTile,
              _product,
            );
    ...
    if (SETTINGS.DebugLevel > 10)
      console.log(`init(): NO TILES TO PARSE ON THIS SITE => SKIP`);
  }

  if (AUTO_SCAN_IS_RUNNING) return;

  const _searchbarContainer = document.getElementById(
  ...
  _searchbarContainer.appendChild(
    createNavButton(
      "ave-btn-favorites",
      translation[langCode]["ave-btn-favorites"],
      "",
  ...
  _searchbarContainer.appendChild(
    createNavButton(
      "ave-btn-list-new",
      translation[langCode]["ave-btn-list-new"],
      "ave-new-items-btn",
  ...
  _searchBarInput.setAttribute(
    "placeholder",
    translation[langCode]["ave-input-search-placeholder"],
  );
    ...
    if (_input.length >= 2) {
      ...
      searchInputTimeout = setTimeout(() => {
        database.query(_input.split(" ")).then((_objArr) => {
          if (SETTINGS.DebugLevel > 10)
            console.log(`Found ${_objArr.length} Items with this Search`);
  ...
  const _pageinationContainer =
    document.getElementsByClassName("a-pagination")[0];
  if (_pageinationContainer) {
    ...
    const _aveNextPageButtonText = `${translation[langCode]["ave-btn-next-page"]} <span class="a-letter-space"></span><span class="a-letter-space"></span><span class="larr">→</span>`;
    ...
    if (anchorTag) {
      ...
      _btn.innerHTML = "${translation[langCode]['ave-btn-next-page']}";
    }
